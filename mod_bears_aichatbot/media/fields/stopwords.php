...
class JFormFieldIgnorewords extends TextareaField
{
    ...
    {
        // Load the language file to ensure translations are available
        $lang = Factory::getLanguage();
        $lang->load('mod_bears_aichatbot', JPATH_SITE . '/modules/mod_bears_aichatbot');
        
        // Get the current value
        $value = trim($this->value);
        
        // Check if the value is empty or the untranslated language constant
        if (empty($value) || $value === 'MOD_BEARS_AICHATBOT_IGNORE_WORDS_DEFAULT') {
            // Use the hardcoded default directly
            $value = $this->defaultIgnoreWords;
        }
        
        // Set the value for the parent to render
        $this->value = $value;
