<?php
/**
 * Custom field for ignore words with proper default value handling
 */
...
class JFormFieldIgnorewords extends TextareaField
{
    ...
    /**
     * Method to get the field value for saving
     *
    ...
    {
        // If the value is the language constant, replace with actual words
        if ($value === 'MOD_BEARS_AICHATBOT_IGNORE_WORDS_DEFAULT' || empty($value)) {
            return $this->defaultIgnoreWords;
        }
    ...
    protected function processValue()
    {
        ...
        // If the value is empty or contains the language constant, use the default
        if (empty($currentValue) || 
            $currentValue === 'MOD_BEARS_AICHATBOT_IGNORE_WORDS_DEFAULT' ||
            strpos($currentValue, 'MOD_BEARS_') === 0) {
            $this->value = $this->defaultIgnoreWords;
        }
    }
}
