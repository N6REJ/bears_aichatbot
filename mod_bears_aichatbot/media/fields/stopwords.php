...
class JFormFieldIgnorewords extends TextareaField
{
    ...
    protected function getInput()
    {
        ...
        // Get the default value from the field definition or language string
        $default = $this->element['default'] ?? 'MOD_BEARS_AICHATBOT_IGNORE_WORDS_DEFAULT';
        
        // If the value is empty, use the default
        if (empty($value)) {
            // Try to translate the default if it's a language constant
            if (strpos($default, 'MOD_') === 0) {
                $translatedDefault = Text::_($default);
                // If translation succeeded, use it
                if ($translatedDefault !== $default) {
                    $value = $translatedDefault;
                } else {
