...
export class ChatBot {
  ...
  init() {
    ...
    if (this.config.connectionCheckInterval > 0 && this.elements.toolbar) {
      const connectionBtn = this.elements.toolbar.querySelector('.connection-btn');
      if (connectionBtn) {
        this.connectionStatus.init(connectionBtn, this.config.ajaxUrl, this.config.connectionCheckInterval);
        this.instance._connectionStatus = this.connectionStatus;
        
        // Set up connection status update callback
        this.connectionStatus.onStatusChange = (isOnline) => {
          connectionBtn.innerHTML = this.getButtonIcon('connection');
          connectionBtn.classList.toggle('online', isOnline);
          connectionBtn.classList.toggle('offline', !isOnline);
          connectionBtn.title = isOnline ? 
            this.getLanguageString('MOD_BEARS_AICHATBOT_STATUS_ONLINE', 'Connected') :
            this.getLanguageString('MOD_BEARS_AICHATBOT_STATUS_OFFLINE', 'Offline');
        };
      }
    }
  }
  ...
  getButtonIcon(type, enabled = false) {
    const icons = {
      connection: this.connectionStatus.isOnline() ? 
